...
async def promote(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await update.message.reply_text("⭐ " + sc(f"User promoted to admin.") + (f" ({sc('title')}: {title})" if title else ""))
    except (BadRequest, TelegramError) as e:
        await update.message.reply_text(f"{sc('Error')}: {e.message}")
...
async def info(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        db = get_db()
        warn_count = await db.warnings.count_documents({"group_id": cid, "user_id": target})
        status_map = {
        ...
        text = (
            ...
            f"━━━━━━━━━━━━━━━━\n"
            f"🆔 *{sc('ID')}:* `{user.id}`\n"
            f"👤 *{sc('Name')}:* {sc(user.first_name or '')}"
            + (f" {sc(user.last_name)}" if user.last_name else "") + "\n"
            f"📛 *{sc('Username')}:* {uname}\n"
            f"🔰 *{sc('Status')}:* {status}\n"
...
async def report(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        reported_link = f"[{sc(reported_user.first_name)}](tg://user?id={reported_user.id})"
        text = (
            f"🚨 *{sc('Report')}*\n"
            f"━━━━━━━━━━━━━━━━\n"
            f"👤 *{sc('Reported by')}:* {reporter_link}\n"
            f"🎯 *{sc('Reported user')}:* {reported_link}\n"
            f"📝 *{sc('Reason')}:* {sc(reason) if reason != sc('No reason given') else sc('No reason given')}\n\n"
            f"📣 {sc('Admins')}: {admin_mentions}"
        )
        await update.message.reply_text(text, parse_mode="Markdown")
        await log_action(context.bot, cid, reported_user.id, reporter.id, f"reported: {reason}")
    except (BadRequest, TelegramError) as e:
        await update.message.reply_text(f"{sc('Error')}: {e.message}")
